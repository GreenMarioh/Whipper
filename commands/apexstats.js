...
module.exports = {
    ...
    callback: (message, arguments, text) => {
        ...
        fetch(`https://api.mozambiquehe.re/bridge?version=5&platform=PC&player=${username}&auth=3AK36upGBaXIzUydMd7W`)
.then(res => res.json())
  .then(data => {
    if (data.global == null) {message.channel.send("Enter a valid origin ID")} else {
      ...
      .setFooter(`Invoked by ${message.author.username}#${message.author.discriminator}`, avatar)
